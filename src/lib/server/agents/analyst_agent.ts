...
3. **심사 소견 작성** (\`synthesis_reasoning\`):
   ...
   - 예시 (1단계 존재 시): "현재 지분율 25% 이상을 보유한 실소유주는 홍길동(30%), 김철수(25%) 등 총 2명으로 식별됨. 해당 주주들의 식별 정보가..."
   - 예시 (2단계 적용 시): "현재 지분율 25% 이상 주주는 존재하지 않으며, 이에 따라 가장 높은 지분율을 보유한 홍길동(20%)을 '최대주주 (25% 미만)'로 식별함."
   - 객관적 사실 기반으로 작성하고, 추측 시에는 "~가능성 있음", "~추정됨" 등 표현 사용
   - **식별정보 평가 기준 (중요)**:
     - 개인 주주: **생년월일(YYYY-MM-DD)**만 있으면 충분한 식별 정보로 간주함. 주민등록번호 전체가 없다고 해서 "데이터 불완전"이나 "식별 제한"으로 평가하지 말 것.
     - 법인 주주: **사업자번호** 또는 **법인등록번호** 중 하나만 있으면 충분함.
   - **보정 표시**: normalization_notes에 "성명 오타 교정" 또는 "성명 의심" 기록이 있는 주주는, 성명 뒤에 **'(확인 필요)'**를 덧붙여 출력할 것 (예: "홍성준(확인 필요)").
